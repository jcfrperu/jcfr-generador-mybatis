...
public class CRUDGeneratorPostgre {

    // parametros obligatorios para configurar
    private final static String RUTA_OUT_BASE = "/home/juaneco/jcfr/temp/generador-postgresql/";
    private final static String RUTA_ARCHIVO_PLANTILLA_BASE = "/home/juaneco/jcfr/desarrollo/git/jcfr-generador-mybatis/src/main/resources/";

    private final static String TABLE_OWNER = "juaneco";
    private final static String NOMBRE_BASE_DATOS = "nombres";

    private final static String PAQUETE_BASE = "com.jcfr.migradornombres";

    // por defecto deber ser false, pero poner true si hay campos upper case dado que postgree es case sensitive
    private final static boolean ENCERRAR_EN_COMILLA_CAMPO_DE_BD = true;
    ...
    private final static String PAQUETE_ENTIDADES = PAQUETE_BASE + ".entidades";
    private final static String PAQUETE_NEGOCIO = PAQUETE_BASE + ".negocio";
    private final static String PAQUETE_SERVICE = PAQUETE_BASE + ".servicios";
    private final static String PAQUETE_SERVICE_IMPL = PAQUETE_SERVICE + ".impl";
    private final static String PAQUETE_CONTROLLER = PAQUETE_BASE + ".controladores";
    ...
    private final static String RUTA_OUT_CONTROLLERS = RUTA_OUT_BASE + PAQUETE_CONTROLLER.replace('.', '/') + "/";
    ...
    private final static String RUTA_ARCHIVO_PLANTILLA_CONTROLLER = RUTA_ARCHIVO_PLANTILLA_BASE + "template-controller.template";
    ...
    public static void main(String[] args) {
        ...
        CRUDGeneratorPostgre instance = new CRUDGeneratorPostgre();
    ...
    public String nextSecError() {
        ...
        if (secuencia_exception > 99) return secuencia_exception + "";
        if (secuencia_exception > 9) return "0" + secuencia_exception + "";
    ...
    private String encerrarCampoBD(String campoBD) {
        ...
        if (ENCERRAR_EN_COMILLA_CAMPO_DE_BD) {
            ...
            // tieneUpper = true;
            if (tieneUpper) return comilla + campoBD + comilla;
    ...
    public void crearDaoXML() {
        ...
        List<CRUDCampoTablaClase> listaCampos = null;

        try {
            String tablaFinal = null;
            ...
            for (CRUDTablaClase itemTabla : listaTablas) {
                ...
                for (CRUDCampoTablaClase itemCampo : listaCampos) {
                    ...
                    if (!itemCampo.isEsCampoClave()) continue;
    ...
    public String tipoBuscarLikeEqualRango(String tParam, Long escala) {
        ...
        if (tParam.startsWith("double")) {
            tablaFinal += "rango-decimal";
        }

        if (tParam.startsWith("character")) {
            tablaFinal += "like";
        }

        if (tParam.startsWith("date")) {
            tablaFinal += "rango-fecha";
        }

        if (tParam.startsWith("timestamp")) {
            tablaFinal += "rango-fecha";
        }
    ...
    public void crearServicioImpl() {
        ...
        try {
            ...
            for (CRUDTablaClase itemTabla : listaTablas) {
                ...
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-SBI-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                ...
                tablaFinal += "} catch (Exception sos) {";
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-SEL-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "log.error(msgError);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "throw handleError(sos);";
                ...
                tablaFinal += "} catch (Exception sos) {";
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-SBM-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "log.error(msgError);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "throw handleError(sos);";
                ...
                tablaFinal += "} catch (Exception sos) {";
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-SBG-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "log.error(msgError);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "throw handleError(sos);";
                ...
                tablaFinal += "} catch (Exception sos) {";
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-INS-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "log.error(msgError);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "throw handleError(sos);";
                ...
                tablaFinal += "} catch (Exception sos) {";
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-UPD-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "log.error(msgError);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "throw handleError(sos);";
                ...
                tablaFinal += "} catch (Exception sos) {";
                tablaFinal += salto + salto + tabular + tabular + tabular;
                tablaFinal += "String msgError = handleMsgError(\"" + firstChar + "SI-DEL-" + nextSecError() + "\", sos);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "log.error(msgError);";
                tablaFinal += salto + tabular + tabular + tabular;
                tablaFinal += "throw handleError(sos);";
    ...
    public void crearBean() {
        ...
        try {
            ...
            for (CRUDTablaClase itemTabla : listaTablas) {
                ...
                tablaFinal += salto;
                tablaFinal += "import java.util.Date;";
                tablaFinal += salto;
                ...
                for (CRUDCampoTablaClase itemCampo : listaCampos) {
                    ...
                    // ignorar los campos de auditoria
                    if (itemCampo.isEsAuditoria()) continue;
    ...
    @SuppressWarnings("unused")
    public void crearControllers() {

        String contenidoFichero;
        String contenidoPlantilla;
        try {
            contenidoPlantilla = FileUtils.readFileToString(new File(RUTA_ARCHIVO_PLANTILLA_CONTROLLER));
        } catch (IOException e1) {
            System.out.println("no se pudo leer el archivo... acabando todo");
            return;
        }
        ...
        try {
            ...
            for (CRUDTablaClase itemTabla : listaTablas) {
                ...
                escribir(RUTA_OUT_CONTROLLERS + "Registro" + tabla.substring(0, 1).toUpperCase() + tabla.substring(1).toLowerCase() + "Controller.java", contenidoFichero);
    ...
    public String prefijoHTML(String tParam, Long escala) {
        ...
        if (tParam.equals("character varying") || tParam.equals("character")) {
            result += "txt";
        }

        if (tParam.startsWith("timestamp") || tParam.startsWith("date")) {
            result += "fec";
        }
    ...
    private String generarInputMask(CRUDTablaClase itemTabla) throws Exception {
        ...
        for (CRUDCampoTablaClase itemCampo : listaCampos) {
            ...
            } else if (tParam.equals("character")) {
            ...
            } else if (tParam.startsWith("timestamp") || tParam.startsWith("date")) { // ok
            ...
            } else if (tParam.equals("integer") || tParam.equals("smallint")) { // ok
    ...
    public String maxLengthHTML(CRUDCampoTablaClase itemCampo) {
        ...
        if (tParam.equals("numeric")) {
            ...
            if (escala == null || escala.intValue() <= 0) {
                ...
                if (precision != null) result = String.valueOf(precision.intValue());

            } else {

                int size = 1;
                if (precision != null) size += precision.intValue();
                if (escala != null) size += escala.intValue();
        ...
        } else if (tParam.equals("character") || tParam.equals("character varying")) {

            if (longitud != null) result = String.valueOf(longitud.intValue());
    ...
    public String tipoDatoJDBC(String tParam, Long escala) {
        ...
        if (tParam.startsWith("double")) result += "DECIMAL";
        if (tParam.startsWith("character")) result += "VARCHAR";
        if (tParam.startsWith("date")) result += "DATE";
        if (tParam.equals("timestamp")) result += "TIMESTAMP";
        if (tParam.equals("integer")) result += "INTEGER";
        if (tParam.equals("bigint")) result += "BIGINT";
    ...
    public List<CRUDTablaClase> listarTablas() throws Exception {
        ...
        if (cache.get("listaTablas") == null) {
            ...
            ResultSet rs = cmd.executeQuery("SELECT tablename FROM pg_catalog.pg_tables where tableowner = '" + TABLE_OWNER + "' and schemaname = '" + ESQUEMA_POSTGRE + "'");
    ...
    public List<CRUDCampoTablaClase> listarCampos(String nombreTabla) throws Exception {
        ...
        if (cache.get("camposTabla" + nombreTabla) == null) {
            ...
            // buscando todos los campos de la tabla JCFR_AQUI MAS NUMERIC_PRECISION CHARACTER_MAXIMUM_LENGTH
            cmd = cnx.createStatement();
            rs = cmd.executeQuery("SELECT column_name, data_type, numeric_scale, numeric_precision, character_maximum_length FROM information_schema.columns WHERE table_catalog = '" + NOMBRE_BASE_DATOS + "' and table_schema = '"
                    + ESQUEMA_POSTGRE + "' AND table_name = '" + nombreTabla + "' ORDER BY ordinal_position");
            ...
            while (rs.next()) {
                ...
                campoTabla = StringUtils.trimToEmpty(rs.getString(1));
                ...
                campoTablaClase.setTipo(StringUtils.trimToEmpty(rs.getString(2)));
                campoTablaClase.setEscala(rs.getLong(3));
                // campos adicionales: precision cuando es numero, y longitud
                // cuando es cadena
                campoTablaClase.setPrecision(rs.getLong(4));
                campoTablaClase.setLongitud(rs.getLong(5));
            ...
            cmd = cnx.createStatement();
            rs = cmd.executeQuery("SELECT a.attname FROM pg_index i JOIN   pg_attribute a ON a.attrelid = i.indrelid   AND a.attnum = ANY(i.indkey) WHERE  i.indrelid = '" + nombreTabla
    ...
    public List<CRUDCampoTablaClase> buscarCamposClave(CRUDTablaClase itemTabla) throws Exception {
        ...
        if (CollectionUtils.isNotEmpty(listaCampos)) {
            ...
            for (CRUDCampoTablaClase campo : listaCampos) {
                ...
                if (campo.isEsAuditoria()) continue;
